...
class PayooPayment
{
	...
	*/
	...
	public function createRequestUrl($order_no, $order_ship_date, $order_ship_days, $order_cash_amount, $chi_tiet_don_hang, $aCustomerInfo = null)
	{
		$Cus_Name = $aCustomerInfo['customer_name'];
		$Cust_Phone = $aCustomerInfo['customer_phone'];
		$Cus_Address = '';
		$Cus_City = '';
		$Cus_Email = $aCustomerInfo['customer_email'];
		$validity_time =  date('YmdHis', strtotime('+1 day', time()));
		
		$str='<shops>
				<shop>
					<session>'.$order_no.'</session>
					<username>'.$this->vdt.'</username>
					<shop_id>'.$this->shop_id.'</shop_id>
					<shop_title>'.$this->shop_title.'</shop_title>
					<shop_domain>'.$this->shop_domain.'</shop_domain>
					<shop_back_url>'.$this->shop_back_url.'</shop_back_url>
					...
					<order_description>'.urlencode($chi_tiet_don_hang).'</order_description>
		...
		$html = '';
		//$html .= $chi_tiet_don_hang;
		$html .= "<script src="._Link_Script_Paynow."></script>";
		$html .= "<form id='frmPayByPayoo' class='test-form'  name='frmPayByPayoo' action='" .$checkouturl. "' method='POST'>
